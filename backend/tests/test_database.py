...
async def test_database_query_execution(mock_db):
    ...
    mock_db.fetch_all.return_value = [{"id": 1, "name": "Item 1"}, {"id": 2, "name": "Item 2"}]
...
async def test_database_insert(mock_db, sample_request_data):
    ...
    async def insert_item(data: dict, db):
        await db.execute(
            "INSERT INTO items (name, description, price) VALUES (?, ?, ?)", data["name"], data["description"], data["price"]
        )
        await db.commit()
...
async def test_database_update(mock_db):
    ...
    async def update_item(item_id: int, data: dict, db):
        await db.execute("UPDATE items SET name = ?, price = ? WHERE id = ?", data["name"], data["price"], item_id)
        await db.commit()
        return {"success": True}
...
async def test_database_pagination(mock_db):
    ...
    mock_db.fetch_all.return_value = [{"id": i, "name": f"Item {i}"} for i in range(1, 11)]

    async def fetch_paginated(page: int, page_size: int, db):
        offset = (page - 1) * page_size
        return await db.fetch_all(f"SELECT * FROM items LIMIT {page_size} OFFSET {offset}")
...
async def test_database_migration_check():
    ...
    def check_migration_status():
        return {"current_version": "001", "latest_version": "001", "needs_migration": False}
